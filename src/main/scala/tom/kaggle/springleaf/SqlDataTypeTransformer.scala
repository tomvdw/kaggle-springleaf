...
object SqlDataTypeTransformer {
  def castColumn(column: String, dataType: DataType): List[String] = {
    dataType match {
      ...
      case default => List(s"$column AS STR_$column")
    }
  }
  ...
  def extractStandardDateFields(column: String): List[String] = {
    List(decimal(extractDateField(column, 3), s"DEC_${column}_DATE_YEAR"),
      string(extractDateField(column, 2), s"STR_${column}_DATE_MONTH"),
      decimal(extractDateField(column, 1), s"DEC_${column}_DATE_DAY"),
      decimal(extractDateField(column, 4), s"DEC_${column}_DATE_HOUR"))
  }
  ...
  def extractDecimal(column: String): List[String] = {
    List(s"CASE WHEN $column IS NULL OR $column = '' THEN NULL ELSE cast($column AS DECIMAL) END as ${"DEC_" + column}")
  }

  def extractBoolean(column: String): List[String] = {
    List(s"CASE WHEN $column = 'false' THEN 0 WHEN $column = 'true' THEN 1 ELSE NULL END AS ${"BOOL_" + column}")
  }
}